...
const getLogKey = () => `offlineLogs-${localStorage.getItem("userId") || "unknown"}`;
...
export const cacheLogEntry = async (entry) => {
  const current = (await localforage.getItem(getLogKey())) || [];
  current.push(entry);
  await localforage.setItem(getLogKey(), current);
};
...
export const syncLogs = async () => {
  const allLogs = (await localforage.getItem(getLogKey())) || [];
  ...
  try {
    ...
    await localforage.removeItem(getLogKey());
  } catch (err) {
